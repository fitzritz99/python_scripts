...
#        print (f)
        ...
        with open(dirpath + f) as file_:
            for chunk in lines_per_n(file_, 1):
                ...
                vtype = ''
                for item in jfile['data']['path']:
                    if 'x' in item['measurements'][0].keys():
                        type = item['measurements'][0]['x']
                    parse_json.append({
                    'Id': item['identity']['Id']
                    ,'coordinates': item['measurements'][0]['location']['coordinates']
                    ,'a': item['measurements'][0]['location']['a']
                    ,'b': item['measurements'][0]['location']['b']
                    ,'c': item['measurements'][0]['c']
                    ,'type': type
                    })
#                    print (item)
...
data_df = pd.DataFrame(parse_json) #,columns=['Id','temp','coordinates','a','b','c']
data_df['lon'] = data_df.coordinates.apply(lambda x: x[0])
data_df['lat'] = data_df.coordinates.apply(lambda x: x[1])
#data_df.to_csv('data_combined.csv') #save output